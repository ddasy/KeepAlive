...
enum CrossKeepalivePolicy {
    ...
    // 把目标时刻向上取整到网格点；相位由已知的窗口边界反推（边界必在网格上）。
    // 观测到的边界带 ±1s 抖动（01:09:59 / 01:10:00），先抹平到整分，
    // 否则会对齐到网格点前 1 秒开火，反被向下取整回上一个网格，白丢 30 分钟。
    static func alignedUp(_ target: Date, anchor: Date?) -> Date {
        guard let anchor else { return target }
        let phase = (anchor.timeIntervalSince1970 / 60).rounded() * 60
