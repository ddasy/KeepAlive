...
可在 30–145 分钟之间以 5 分钟调整，也可选择 **居中**，让两侧窗口起点尽量相隔 **150 分钟（2 小时 30 分）**；关闭后恢复原来的到期续窗。此功能仅在两侧保活均开启时生效。
...
- 选择 **居中** 时，到期一侧以对方最新窗口起点之后 150 分钟为目标：Codex 精确等待到该时刻，Claude 按 `windowEnd` 推导的半小时网格取最近点（正好半格取较早点）。目标网格已过去时立即触发。
- 居中模式从任意初始错开量最多各等待一次；随后两侧不会每轮重复等待，实际窗口起点间隔保持在约 150 分钟附近（Claude 受网格影响约 ±15 分钟，Codex 无网格偏差）。
...
阈值上限低于 150 分钟，因为两个 5 小时周期稳定错开时，相邻间隔之和为 5 小时；居中模式直接以这 150 分钟的中点为目标。
...
点击「刷新」右侧的「设置」，在首页原有内容下方展开设置；再次点击即可收起；关闭菜单栏弹窗后，下次打开时设置默认收起。屏幕能容纳时直接完整显示，不使用滚动容器；仅在内容超出屏幕可用高度时启用滚动，不再切换独立页面。
Claude / Codex 保活、交叉保活及间隔、开机自启、自动查询、自动排序、隐藏倒计时按原始顺序显示在「设置」上方。
展开后，「设置」下方先显示「已隐藏的控件」（有隐藏项时），然后显示「控件显示」。显示控制采用与现有功能开关一致的勾选样式：勾选会将控件移回上方原来的位置，取消勾选则移入「已隐藏的控件」，每个控件只显示一次。收起设置时，下方区域一并收起；隐藏不会关闭功能。
已有显示配置直接沿用，包括部分隐藏和全部隐藏；仅在没有保存过配置时默认全部显示。

「菜单栏进度条」可分别调节时间下方用量条的填充色和底色深度（0–100% 不透明度），并提供预览和恢复默认按钮。
默认填充色为 100%、底色为 22%，保留原有绿／橙／红用量阈值。显示选项和颜色深度会保存，重启后继续生效。

保活时间、上次保活结果及暂停／周限／进行中状态集中显示，不单独显示「保活记录」标题。开启交叉保活时，记录位于交叉时间摘要下方；关闭交叉保活时，记录仍可在设置中查看。同一时间只展示一份记录，Claude / Codex 记录顺序跟随用量卡片的实际显示顺序（包括手动和自动排序），各自的时间与结果一起排列。查询或保活错误（包括请求成功但未确认新窗口）仍直接显示，后续保活成功后清除对应错误提示。

开启交叉保活后，用量下方始终显示交叉时间摘要：设定间隔、当前实际间隔或等待截止时间，下方显示保活记录，不再显示两侧的周期开启时间。隐藏交叉保活快捷开关或收起设置不会隐藏该摘要。
