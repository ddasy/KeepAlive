...
在弹窗的「设置 → 保活」区域开启 **交叉保活**，设置两侧周期起点的最小间隔，默认 **120 分钟**。
...
点击「刷新」右侧的「设置」进入设置页，点击「返回」回到用量首页。
Claude / Codex 保活、交叉保活及间隔、开机自启、自动查询、自动排序、隐藏倒计时均在设置中操作。
这些开关默认不显示在首页；在「菜单栏弹窗显示」中可逐项重新显示。隐藏只影响快捷入口，不会关闭功能。

「菜单栏进度条」可分别调节时间下方用量条的填充色和底色深度（0–100% 不透明度），并提供预览和恢复默认按钮。
默认填充色为 100%、底色为 22%，保留原有绿／橙／红用量阈值。显示选项和颜色深度会保存，重启后继续生效。

保活时间、上次保活结果及暂停／周限／进行中状态集中显示在「设置 → 保活记录」。菜单栏弹窗首页仅直接显示查询或保活错误（包括请求成功但未确认新窗口），后续保活成功后清除对应错误提示。
